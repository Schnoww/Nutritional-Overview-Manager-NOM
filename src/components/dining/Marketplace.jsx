...
const Marketplace = () => {
	...
	const filteredBreakfast = iniData.filter((food) => food.location === "Marketplace" && food.time === "Breakfast");
	const filteredLunch = iniData.filter((food) => food.location === "Marketplace" && food.time === "Lunch");
	const filteredDinner = iniData.filter((food) => food.location === "Marketplace" && food.time === "Dinner");
	// const filterdBreak = iniData.filter
		...
		<div className="w-full flex justify-center p-6 bg-gray min-h-screen overflow-y-auto">
			<div className="flex-1 overflow-auto relative z-10">
				...
				<div classname="flex-1">
					...
					<h2 classname = "text-3xl font-bold text-white text-left mb-6">Breakfast</h2>
					{filteredBreakfast.map((food) => (
						<div
				...
				<h1 classname = "text-4xl font-bold text-white text-center mb-6">Lunch</h1>
				{filteredLunch.map((food) => (
					<div
						key={food.name} // Key for the list item
						className="flex items-start gap-4 p-4 border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition"
						onClick={() => toggleExpand(food.name)} // Toggle expansion on click
				...
				<h1 classname = "text-4xl font-bold text-white text-center mb-6">Dinner</h1>
				{filteredDinner.map((food) => (
					<div
						key={food.name} // Key for the list item
						className="flex items-start gap-4 p-4 border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition"
						onClick={() => toggleExpand(food.name)} // Toggle expansion on click
